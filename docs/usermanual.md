...
## Implementation of a Robot communication microservice
 
#### Subscription
* The **robot communication microservice** subscribes to the program manager by:

```
curl -X POST http://[program_manager_ip]:81/Application/AddRobotCommunicationService -H 'Content-Type: application/json' -d '{"robotType":"New_RobotType","DownloadToRobot_Url":"download_url_new_robobType", "UploadFromRobot_Url":"upload_url_new_robobType"}'
```
#### Download of a program to the robot
* The **robot communication microservice** is requested to donwload a robot executable to the robot with a POST request to [download_url_new_robobType] with a **robot program** json in the body of the request.

#### Upload of a program to the FIREWARE
* The **robot communication microservice** is requested to upload a robot executable to the robot with a POST request to [upload_url_new_robobType] with a {"programId" = "my_programID"} json in the body of the request.
* The **robot communication microservice** sends a Patch request to the URL:
```    
[fireware_ip_address]:[port]/v2/entities/my_programID/attrs
```    
with the json {"programCode" = {"value": "programCodeEncodedInBase64","type": "Text"} } in the body of the request